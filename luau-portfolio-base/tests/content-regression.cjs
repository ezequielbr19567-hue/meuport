...
(async()=>{
 ...
 const route=load('app/api/content/route.ts',{'@/lib/contentStore':{getPortfolioContent:async()=>{throw Error('offline')}}});
 ...
 const okRoute=load('app/api/content/route.ts',{'@/lib/contentStore':{getPortfolioContent:async()=>good}});assert.equal((await (await okRoute.GET()).json()).source,'saved');
 ...
 const realFetch=global.fetch;global.fetch=async()=>new Response(JSON.stringify({content:good}),{status:200});await assert.rejects(()=>client.requestContent(new AbortController().signal));global.fetch=realFetch;
