...
export async function getPortfolioContent(): Promise<PortfolioContent> {
  ...
  // the public client can retry or use its last known-good snapshot instead.
  if (!supabase) {
  ...
  // No row means the owner has not saved custom content yet, so the bundled
  // template is the intentional initial state. This is different from an error.
