...
import { readConfirmedContent, rememberContent, requestContent } from "@/lib/contentClient";
...
export default function PortfolioClient() {
  ...
  useEffect(() => {
    ...
    return () => { controller.abort(); clearTimeout(retryTimer); window.removeEventListener("online", load); };
  }, [contentAttempt]);
  ...
  if (!contentLoaded && contentError) {
    return <main className="contentRecovery"><div className="panel">
      ...
      <button className="button primary" onClick={() => setContentAttempt(n => n + 1)}>{language === "pt" ? "Tentar novamente" : "Try again"}</button>
    </div></main>;
  }
